class Canary < Achievement
  ...
  # Comment out below for once-off achievements
   def self.award_achievements_for(user)
     ...
     achievement = Achievement.new(:user => user, :kind => "Canary", :level => nil)
     achievement.save!
   end
   ...
   def self.small_image
    'badges/small/canary.jpg'
   end
   
   def self.description
   	'Submit a text campaign for a media outlet to obtain this badge.'
   end
